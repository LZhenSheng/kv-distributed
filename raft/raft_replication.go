...
type LogEntry struct {
	Term         int         //the log entry's position in the log
	CommandValid bool        //if it should be applied
	Command      interface{} //the command should be applied to the state mechine
}
type AppendEntriesArgs struct {
	Term     int
	LeaderId int

	//used to probe the match
	PrevLogIndex int
	PrevLogTerm  int
	Entries      []LogEntry

	// used to update the follower's commit
	LeaderCommit int
}
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if rf.log[args.PrevLogIndex].Term != args.PrevLogTerm {
		LOG(rf.me, rf.currentTerm, DLog2, "<- %d, Reject log, Prev log not match, [%d]: T%d != T%d", args.LeaderId, args.PrevLogIndex, rf.log[args.PrevLogIndex].Term, args.PrevLogTerm)
		return
	}
